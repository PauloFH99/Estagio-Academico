...
public class DALProduto {
    ...
    public boolean salvar(Produto p) {
        String sql = "insert into produto (descricao,estq_min,estq_max,preco,ativo,estoque_fisico,cod_categoria) values('#1','#2','#3','#4','#5','#6',#7)";
        //sql=sql.replace("#0",""+1);
    ...
    public boolean alterar(Produto p) {
        String sql = "update produto set descricao='#1',estq_min = '#2',estq_max = '#3',preco='#4', estoque_fisico='#5', cod_categoria=#6 where codigo=" + p.getCodigo();
        sql = sql.replace("#1", p.getDescricao());
        sql = sql.replace("#2", "" + p.getEst_min());
        sql = sql.replace("#3", "" + p.getEst_max());
        sql = sql.replace("#4", "" + p.getPreco());
        ...
        try {
            while (rs.next()) {
                if (rs.getBoolean("ativo") != false) {
                    prod.add(new Produto(rs.getInt("codigo"), rs.getInt("estq_min"), rs.getInt("estq_max"), rs.getString("descricao"), rs.getDouble("preco"), rs.getInt("estoque_fisico"), rs.getBoolean("ativo"), dalc.get(rs.getInt("cod_categoria"))));
                }
            }
    ...
    public Produto get(int cod) {
        ...
        try {
            if (rs.next()) {
                prod = new Produto(rs.getInt("codigo"), rs.getInt("estq_min"), rs.getInt("estq_max"), rs.getString("descricao"), rs.getDouble("preco"), rs.getInt("estoque_fisico"), rs.getBoolean("ativo"), dalc.get(rs.getInt("cod_categoria")));
            }
        } catch (SQLException ex) {
    ...
    public Produto getQtde(Integer codigo) {
        ...
        try {
            if (rs.next()) {
                return new Produto(rs.getInt("codigo"), rs.getInt("estq_min"), rs.getInt("estq_max"), rs.getString("descricao"), rs.getDouble("preco"), rs.getInt("estoque_fisico"), rs.getBoolean("ativo"), dalc.get(rs.getInt("cod_categoria")));
